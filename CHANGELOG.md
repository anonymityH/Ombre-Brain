...
- **不连续事件证据显示明确片段边界。** `source_read(scope="event")` 在同一记忆引用
  多段原文时，会在每段前标出片段序号和原文行号，避免将跳跃的证据范围伪装成一段
  连续对话；单一连续范围仍逐字返回，不添加展示标记。
- **按真实事件时间浏览历史。** `breath_search` 和 `breath_advanced` 的日期过滤优先使用
  `event_time`，旧桶无事件时间时才回退 `created`；带日期的 catalog 会返回核验所需的
  精确定位信息，普通浮现仍不会注入原文。
- **兼容 ChatGPT 浏览器插件导出的 Markdown。** 独立的 `User:` / `GPT:` 标记会被
  解析为真实轮次，`message time:` 保留为事件时间而不混入正文；导出来源头和消息间
  分隔线会被剥离。插件未提供的 GPT 时间保持未知，分块只使用已知时间构造首尾范围。
- **导入证据从 chunk 级收窄到事件轮次级。** 每个规范化 chunk 会生成只含可信轮次号、
  行范围和来源时间的 manifest；提取模型必须为每条记忆选择直接支持它的
  `source_turns`，导入器再校验并换算成不可伪造的 `source_ranges` 与事件时间。模型漏选
  或乱填时才退回完整 chunk，不允许模型自行编造行号或时间。日期目录与普通检索定位
  现在同时显示不同的 `event_time_end`，不再把一个时间窗伪装成单点时刻。
